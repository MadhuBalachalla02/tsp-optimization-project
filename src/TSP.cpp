...
const double GLOBAL_TIME_LIMIT_SAFETY =
    MAX_RUNTIME_SECONDS - 2.0; // Stop at 58.0 seconds
// The number of nearest neighbors to consider for any move (LKH standard is
// 15-20)
...
void outputSolutionToFile(const vector<int> &final_tour,
  ...
  // Use ios::app to append or create
  ...
  // Add the return edge (closing the cycle) and a newline
  outfile << ", " << final_tour.front() + 1 << std::endl;

  outfile.close();
  cerr << "Solution appended to: " << filename << endl;
}
...
void precomputeNNList() {
  ...
  cerr << "Precomputed " << K_NN_LIST << "-NN list for all " << N << " nodes."
       << endl;
}
  ...
  while (improved) {
    ...
    for (int i = 0; i < N_nodes; ++i) {
      ...
      for (int k = 0; k < N_nodes; ++k) {
        ...
        // Ensure the two edges (i, i1) and (k, k1) are not adjacent or
        // identical. i != k, i != k1, i1 != k, i1 != k1.
        ...
        if (gain > 1e-9) {
          ...
          if (i1 < k) {
            ...
            // Wrapping segment reversal: Reversing the path from i1 -> ... ->
            // N-1 -> 0 -> ... -> k
...
relocateOpt(vector<int> tour,
  ...
  while (improved) {
    ...
    for (int i = 0; i < N_nodes; ++i) {
      ...
      // Iterate over the K Nearest Neighbors of node 'u' (the node that now
      // connects to w). The NN node 'p_node' will be the node *before* the
      // insertion point. (K iterations)
      for (int p_node : nn_list[u]) {

        int p_idx =
            node_to_index[p_node]; // Index of p_node (insertion predecessor)
        ...
        // Skip if insertion is adjacent to removal, which leads to identity
        // moves or complex wraps
        if (p == v || q == v || p == u || q == u)
          continue;
        ...
        double new_edges_insert =
            D[p][v] + D[v][q]; // Edges created by insertion

        // Total gain calculation:
        // Gain = (Removed Edges) - (New Edges)
        double gain =
            (removed_cost + old_edges_insert) - (D[u][w] + new_edges_insert);

        if (gain > 1e-9) {
          // *** FIRST-IMPROVEMENT: Execute the move immediately ***

          // 1. Remove node v from its current position
          // Must calculate the current index again as tour might have been
          // changed in the loop
          int current_i = node_to_index[v];
          tour.erase(tour.begin() + current_i);

          // 2. Calculate the new insertion index.
          // If current_i < p_idx, erasing 'i' shifts all indices from
          // current_i+1 to p_idx down by 1. If current_i > p_idx, erasing 'i'
          // does not affect indices before p_idx.
...
void readGraph(const std::string &filename) {
  ...
  // --- CORRECT HEADER SKIP ---
  // 1. Read and discard the rest of the line containing 'N' (the newline
  // character).
...
int main(int argc, char *argv[]) {
  ...
  auto start_time =
      std::chrono::steady_clock::now(); // This is the START of the entire
                                        // program (for the final 60s check)

  // 1. Read Graph Data
  readGraph("TSP_1000_euclidianDistance.txt");
  cerr << "--- CHECKPOINT 1: Graph Read Complete (" << N << " nodes) ---"
       << endl;

  // 0. Precompute Nearest Neighbor List (CRITICAL STEP FOR SPEED)
  precomputeNNList();

  // Initialize best tour with a random tour if not provided by
  // nearestNeighbor()
  vector<int> best_tour = nearestNeighborTour();

  double best_cost = calculateTourCost(best_tour);
  cerr << "--- Initial Cost(NN Start): " << fixed << setprecision(2)
       << best_cost << endl;
  ...
  while (std::chrono::steady_clock::now() - start_time <
         std::chrono::duration<double>(GLOBAL_TIME_LIMIT_SAFETY)) {
    ...
    if (iteration_count > 1) {
      // Aggressive Perturbation: Reverse a segment of 5% to 20% of the tour
      // length.
    ...
    if (local_cost < best_cost) {
      ...
      //cerr << "--- New Best Cost (" << iteration_count << "): " << fixed
        //   << setprecision(2) << best_cost << endl;
    }
  }
  // -------------------------------------------------
  ...
  // cout << "Cost: " << std::fixed << std::setprecision(2)<< best_cost << endl;

  // Output file logic
  string output_filename = "solution_915552766.txt";

  // Console Output (Comma separated for consistency)
  cout << "Nodes List:";
  ...
  cerr << "--- Optimization Complete. \n Final Cost: " << fixed
       << std::setprecision(2) << best_cost << endl;

  cout << "Cycles: " << std::scientific << std::setprecision(1)
       << (double)total_moves_checked << endl;

  outputSolutionToFile(best_tour, output_filename);
  auto end_time = std::chrono::steady_clock::now();
  std::chrono::duration<double> duration = end_time - start_time;
  double elapsed_seconds = duration.count();
  cerr << "Total Time: " << fixed << elapsed_seconds << " seconds over "
       << iteration_count << " starts." << endl;
